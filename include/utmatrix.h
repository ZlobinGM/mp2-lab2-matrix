...
{
	if (s < 0 || s > MAX_VECTOR_SIZE) throw "too_large_or_negative_length";
	if (si < 0) throw "negative_startindex";
	Size = s;
	StartIndex = si;
...
{
	if (pos < StartIndex || pos >= Size + StartIndex) throw "too_large_or_negative_index";
...
{
	if (s < 0 || s > MAX_MATRIX_SIZE) throw "too_large_or_negative_length";
	Size = s;
	for (int i = 0; i < Size; i++)
